...
Client.prototype.createSession = function(data,cb){
    this.fetchJSON("/api/sessions/create",JSON.stringify(data),function(err,session){
        if(!err){
            localStorage.session = JSON.stringify(this.session = session);
            cookie.set("session",this.session._id);
        }
        cb(err,session);
    }.bind(this));
}
...
Client.prototype.deleteSession = function(cb){
    ...
    this.fetch("/api/sessions/"+this.session._id+"/delete",null,function(){
        delete this.session;
...
Client.prototype.ensurePermissions = function(permissions,link){
    if(!this.session || !this.session.permissions) return visit(link);
    for(var i = 0; i < permissions.length; i++){
        if(!this.hasPermission(permissions[i])) return visit(link);
    }
}
