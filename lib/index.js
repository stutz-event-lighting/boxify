...
var Boxify = module.exports = function Boxify(config){
    ...
    this.app.post("/api/contacts/create",function(req,res,next){console.log("here");next();},ensurePermission("contacts_write"),bp.json(),require("./routes/contacts/create.js"));
    ...
    this.addPermission("users_write","Neue Benutzer erstellen");
...
Boxify.prototype.start = function(){
    var self = this;
    var db = mongoose.connect("mongodb://"+this.config.dbpath+"/boxify",function(err){
        if(err) throw err;
        ...
        self.update(function(){
            async.each(Object.keys(self.modules),function(module,cb){
                self.modules[module].start().then(cb).catch(cb);
            },function(err){
                if(err) throw err;
...
Boxify.prototype.update = function(cb){
    ...
    var patches = {
        ...
        "0.1.5":function(cb){
            self.db.collection("users").update({},{$set:{ownedBy:[]}},{multi:true},cb);
        },
        "0.1.6":function(cb){
            self.db.collection("equipment").find({},{contents:true}).toArray(function(err,entries){
                async.eachSeries(entries,function(entry,cb){
                    for(var type in entry.contents){
                        if(entry.contents[type].ids) entry.contents[type].ids = entry.contents[type].ids.map(function(id){return id+""})
                    }
                    self.db.collection("equipment").update({_id:entry._id},{$set:{contents:entry.contents}},cb)
                },cb)
            });
        },
        "0.1.7":require("./patches/0.1.7.js"),
        "0.1.8":require("./patches/0.1.8.js")
    };
