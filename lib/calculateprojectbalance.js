...
module.exports = function(db,project,cb){
    async.parallel([
        ...
        function(cb){
            db.collection("equipmentrentals").find({project:project,status:"received"},{items:true,supplier:true}).toArray(cb)
        }
        ...
        var entries = results[0].concat(results[1].map(function(checkin){
        	...
        	return checkin
        }),results[2].map(function(rental){
            rental.checkin = true;
            for(var type in rental.items){
                var entry = {count:rental.items[type],suppliers:{}};
                entry.suppliers[rental.supplier] = {count:rental.items[type]};
                rental.items[type] = entry;
            }
            return rental;
        }));
