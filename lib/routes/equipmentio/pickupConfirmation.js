...
module.exports = function(req,res){
    ...
    req.app.db.collection("equipmentio").findOne({_id:_id},{project:true,time:true,items:true,history:true,person:true},function(err,checkout){
        ...
        async.parallel([
            function(cb){
                req.app.db.collection("projects").findOne({_id:checkout.project},{projectNumber:true,customer:true,name:true,start:true,end:true},cb);
            },
            function(cb){
                req.app.db.collection("equipmenttypes").find({_id:{$in:articleIds}}).toArray(cb)
            },
            function(cb){
                if(!checkout.person) return cb(null);
                req.app.db.collection("contacts").findOne({_id:checkout.person},{firstname:true,lastname:true,emails:true,phones:true},cb);
            }
            ...
            var person = results[2];

            req.app.db.collection("contacts").findOne({_id:project.customer},{type:true,salutation:true,firstname:true,
                lastname:true,streetName:true,streetNr:true,zip:true,city:true,emails:true,phones:true},function(err,customer){
