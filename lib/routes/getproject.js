...
module.exports = function(req,res){
    ...
    async.parallel([
        ...
        function(cb){
            req.app.db.collection("equipmenttransactions").find({project:id,type:"reservation"},{items:true}).toArray(cb);
        }
        ...
        for(var i = 0; i < reservations.length; i++){
            ...
            reservation.count = reservation.items.length;
            reservation.total = reservation.items.map(function(item){return item.count}).concat([0]).reduce(function(prev,val){return prev+val});
