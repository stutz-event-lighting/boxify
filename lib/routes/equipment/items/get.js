...
module.exports = function*(){
    ...
    var data = yield [
        ...
        function*(){
            ...
            var io = yield this.app.db.collection("equipmentio").findOne(query).select("project time type").sort({time:-1})
            if(io && io[0].type == "checkout"){
                var project = yield this.app.db.Project.findOne({_id:io[0].project}).select("customer");
