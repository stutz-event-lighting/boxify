...
module.exports = function(req,res){
    async.parallel([
        ...
        function(cb){
            ...
            req.app.db.collection("equipmentrentals").findOne({_id:mongo.ObjectID(req.params.rental)},{name:true,items:true,status:true,supplier:true},function(err,rental){
                if(err || !rental) return cb(err);
                req.app.db.collection("contacts").findOne({_id:rental.supplier},{firstname:true,lastname:true},function(err,supplier){
                    if(err || !supplier) return cb(err);
                    rental.supplier = {value:rental.supplier,label:(supplier.firstname?supplier.firstname+" ":"")+(supplier.lastname||"")};
                    cb(null,rental);
                });
            });
        },
