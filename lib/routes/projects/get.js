...
module.exports = function(req,res){
    ...
    async.parallel([
        function(cb){
            req.app.db.collection("projects").findOne({_id:id},{_id:true,name:true,customer:true,start:true,end:true,balance:true,remark:true,status:true},function(err,project){
        ...
        function(cb){
            req.app.db.collection("equipmentio").find({project:id},{type:true,time:true,items:true,user:true,draft:true}).sort({time:1}).toArray(cb);
        }
        ...
        var users = Object.keys(util.createIndex(ios,"user")).filter(function(id){return id!="undefined"}).map(function(id){return parseFloat(id)});
