...
module.exports = function(req,res){
    ...
    async.parallel([
        function(cb){
            req.app.db.collection("projects").findOne({_id:id},{_id:true,name:true,customer:true,balance:true,comment:true,status:true},function(err,project){
        ...
        req.app.db.collection("users").find({_id:{$in:users}},{firstname:true,lastname:true}).toArray(function(err,users){
            if(err) return res.fail();
